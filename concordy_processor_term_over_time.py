import nltk, pickle, matplotlib.pyplot as plt, streamlit as st, numpy as np, urllib.request
nltk.download('punkt')
...
if lem_ans == "Yes":
    #pickleIn = open("concordy_lemma.pickle",'rb')
    pickleIn = urllib.request.urlopen("http://wwww.adammazel.com/concordy_raw_no_POS.pickle")
    d_year_month_text = pickle.load(pickleIn)
    search_terms = [lemmatizer.lemmatize(term.lower()) for term in search_terms if term.isalpha()]
else:
    #pickleIn = open("concordy_raw_no_POS.pickle",'rb')
